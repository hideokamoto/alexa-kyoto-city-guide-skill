...
describe('LaunchRequest', () => {
  ...
  it('should talk welcome message and wait user response [English]', () => {
    ...
    const succeed = (data) => {
      ...
      assert.equal(outputSpeech.ssml, '<speak> Welcome to Kyoto Guide! Say about, to hear more about the city, or say recommend a temples, or say, go outside. </speak>')
  ...
  it('should talk welcome message and wait user response [Japanese]', () => {
    ...
    const succeed = (data) => {
      ...
      assert.equal(shouldEndSession, false)
      assert.equal(outputSpeech.type, 'SSML')
      assert.equal(outputSpeech.ssml, '<speak> 京都ガイドへようこそ！「アレクサ、京都ガイドで京都のお寺を教えて」と聞いてみてください。 </speak>')
    }
