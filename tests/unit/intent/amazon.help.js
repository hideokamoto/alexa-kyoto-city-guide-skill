...
describe('AMAZON.HelpIntent', () => {
  ...
  it('should talk about the skill [English]', () => {
    ...
    const succeed = (data) => {
      ...
      assert.equal(outputSpeech.ssml, '<speak> Say about, to hear more about the city, or say recommend a temples, or say, go outside. </speak>')
  ...
  it('should talk about the skill [Japanese]', () => {
    ...
    const succeed = (data) => {
      ...
      assert.equal(shouldEndSession, false)
      assert.equal(outputSpeech.type, 'SSML')
      assert.equal(outputSpeech.ssml, '<speak> このスキルでは、京都のお寺や観光地などを紹介します。「アレクサ、京都ガイドで京都のお寺を教えて」と聞いてみてください。 </speak>')
    }
